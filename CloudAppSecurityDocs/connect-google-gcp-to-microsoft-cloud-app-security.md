...
この記事では、コネクタ Api を使用して Microsoft Cloud App Security を既存の Google Cloud Platform (GCP) アカウントに接続する手順について説明します。 この接続により、GCP の使用を可視化し、制御することができます。 Cloud App Security が GCP を保護する方法の詳細については、「 [gcp を保護](protect-gcp.md)する」を参照してください。
...
## <a name="configure-cloud-app-security"></a>Cloud App Security の構成

* Cloud App Security ポータルで、 **[調査]** 、 **[接続さ]** れたアプリ の順にクリックします。
