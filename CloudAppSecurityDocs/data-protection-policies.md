---
title: "ファイル ポリシー | Microsoft Docs"
...
7.  [**コンテンツ検査方法**] を選択します。 組み込み DLP では、ファイルを内容によってフィルタリングできます。 ファイルの内容をスキャンする場合は、次に [**組み込み DLP**] を選択します。 コンテンツ検査を有効にしたら、事前設定されている式を使用するか、他のカスタマイズされた式 (部分文字列または独自の[正規表現](working-with-the-regex-engine.md)) を検索できます。  
    ...
    選択された式に内容が一致した場合に、違反の通知およびログから一致した内容自体をマスクすることもできます。 チェックされた違反テキストは "X" 文字に置き換えられます。 番号は "#" 文字に置き換えられ、Cloud App Security 内に保存されることはありません。  違反の最後の 4 文字のマスクを解除するためのオプションを選択することもできます。 既定では、違反は完全にマスクされており、コンテキストの違反の前後の 40 文字が示されています。 このオプションにより、違反自体の最後の 4 文字のマスクが解除されます。
